...
\section{手法の概要}
まず, MCLのパーティクルが多いと計算時間もかかってしまうため, ランダムサンプリングを行う.
本論文では, サンプル数100で計算を行う.
%@@@「100」の根拠を統計学の観点から説明できるとベスト

クラスタ数の自動推定アルゴリズムはLastLeapを用いる.
%@@@「は」->「には」
%@@@引用をはやめにつけること
LastLeapは, 事前に任意のクラスタ数でk-meansを用いてクラスタリングし, 
%@@@k-meansでは任意のクラスタ数でクラスタリングできないので、もうちょい説明を。
各クラスタの中心座標を用いてクラスタ数を推定するアルゴリズムである. (LastLeap参考文献)
k-meansは, 最初に与えられたクラスタ数にデータを分類するクラスタリングのアルゴリズムである. (k-means参考文献)
LastLeapで求めた推定クラスタ数が2以上になったとき, パーティクルが分離したと判断する. 
