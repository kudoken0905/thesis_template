...
\section{手法の概要}
まず, MCLのパーティクルが多いと計算時間もかかってしまうため, ランダムサンプリングを行う. 本論文では, サンプル数100で計算を行う

クラスタ数の自動推定アルゴリズムはLastLeapを用いる.
LastLeapは, 事前に任意のクラスタ数でk-meansを用いてクラスタリングし, 各クラスタの中心座標を用いてクラスタ数を推定するアルゴリズムである. (LastLeap参考文献)
